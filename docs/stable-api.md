...
`AbstractMediaClassifier` contract that backends and external plugins implement.

## The classifier object

`load_media_classifier(config=None, voc_match_func=None)` returns an
`AbstractMediaClassifier`. All backends expose the same four methods.
...
orthogonal to `MediaType` and are what the content filter blocks on. Backends that
can cheaply surface genre (keyword, guided, m2v) override this.
...
confidence. The default implementation derives the domain from `classify()`
(non-`GENERIC` type ⇒ `OCP_PLAY`, else `NOT_OCP`). Backends with a dedicated domain
head (M2V, padatious, guided) override it for better accuracy and to detect
`OCP_CONTROL`.
...
This ABC is both the internal backend interface and the **plugin contract** for
external classifiers discovered via the `opm.media.classifier` entry-point group
(see [external-plugins.md](external-plugins.md)).
...
- Override `classify_domain()` when you have a cheap domain head (M2V, padatious,
  sklearn/guided domain model).
- Override `is_ocp_query()` when you also handle `ocp_control` (e.g. padatious), so
  control commands count as OCP queries.
- Override `classify_genres()` when you can surface genre signal (keyword, guided,
  m2v) so the [content filter](content-filtering.md) can block on it.
