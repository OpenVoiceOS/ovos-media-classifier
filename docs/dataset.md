...
dataset exists yet**, so the current build runs on the **fallback** path, `movies.jsonl` carries no person fields and the movie director/writer/actor
...
of type X appears but the correct `media_type` is Y, driven by a context word, e.g. *"the {movie_title} soundtrack"* is **music**, *"the {movie_title} trailer"*
...
filled (so `ner_movie_title=1`), but the row is labelled with the correct type, that mismatch is the hard signal that teaches disambiguation. The disambiguating
...
---
[← Entity lists](entity-lists.md) · [Home](index.md) · [Data sources →](data-sources.md)
