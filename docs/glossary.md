...
| **OCPPlayIntent** | The **internal**, fine-grained label space (e.g. `MUSIC`, `ANIME`, `HENTAI`). Richer than the public taxonomy; it projects onto a `MediaType` + genre tags via `PLAY_INTENT_TO_MEDIA_TYPE` / `PLAY_INTENT_TO_GENRES`. Not part of the public output. |
| **OCPEntityLabel** | The **NER vocabulary** (`artist_name`, `movie_title`, `radio_station`, …). Each label maps to a play intent via `NER_LABEL_TO_PLAY_INTENT`, so an entity hit resolves to a media type. |
