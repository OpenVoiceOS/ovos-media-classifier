...
| **content_genres (the head)** | The real genre(s) (`rock` / `action` / `horror` / …), constrained to `mediavocab.KNOWN_GENRES`. `classify_genres()` returns them. Distinct from the content-form `genres` above (`anime`/`adult`/…) that the filter reads. (`mood` / `era` are no longer modelled — dropped in the mediavocab-axes alignment.) |
...
| **accessibility** | Requested accessibility assets, `mediavocab.AccessibilityKind` (`subtitles` / `audio_description` / `sign_language` / …). Multi-label, `classify_accessibility()`. (`dubbed` has no mediavocab home yet — deferred to Phase 2.) |
...
| **presentation** | Classifier-local picture-presentation placeholder (`black_and_white` / `silent` / `3d`) with no mediavocab home yet — redirected to a `PictureFormat` enum in Phase 2. Multi-label, `classify_presentation()`. |
